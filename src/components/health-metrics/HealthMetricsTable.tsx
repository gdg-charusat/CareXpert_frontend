...
export function HealthMetricsTable({
  ...
  const handleStatusFilter = (status: string) => {
    setFilters(prev => ({
      ...prev,
      status: status === 'all' ? undefined : status as MetricStatus,
      offset: 0,
    }));
    setPage(0);
  };
            ...
            {metrics.length === 0 ? (
              ...
              metrics.map((metric) => (
                <TableRow
                  key={metric.id}
                  className={cn(
                    metric.status === 'CRITICAL' && 'bg-red-50 dark:bg-red-950/20',
                    metric.status === 'ABNORMAL' && 'bg-orange-50 dark:bg-orange-950/20'
                  )}
                >
                  ...
                  <TableCell>
                    <Badge variant="secondary" className={STATUS_COLORS[metric.status]}>
                      {metric.status}
                    </Badge>
                  </TableCell>
                  <TableCell>
